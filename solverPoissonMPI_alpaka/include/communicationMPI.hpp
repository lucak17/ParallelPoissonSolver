            ...
            for(int dir=0; dir< DIM; dir++)
            {
                ...
                else if(dir==2)
                {
                    if(hasCommunication_[2*dir])
                    {
                        ...
                        globalLocationOther[dir]--;
                        int rankOther=globalLocationOther[0] + globalLocationOther[1]*nranks_[0] + globalLocationOther[2]*nranks_[0]*nranks_[1];
                        int numElementsCommZ = stride_j*nlocal_noguards_[1] - guards_[0];
                        std::array<int,3> arrayIndexGuards;
                        k=indexLimitsComm[4];
                        j=indexLimitsComm[2];
                        i=indexLimitsComm[0];
...
template<int DIM, typename T_data>
class CommunicatorMPIAlpaka{
    public:

    CommunicatorMPIAlpaka(const BlockGrid<DIM,T_data>& blockGrid, const int stride_j, const int stride_k):
        blockGrid_(blockGrid),
        nranks_(blockGrid.getNranks()),
        globalLocation_(blockGrid.getGlobalLocation()),
        origin_(blockGrid.getOrigin()),
            ...
            for(int dir=0; dir< DIM; dir++)
            {
                ...
                else if(dir==2)
                {
                    if(hasCommunication_[2*dir])
                    {
                        ...
                        int numElementsCommZ = stride_j_*nlocal_noguards_[1] - guards_[0];
                        std::array<int,3> arrayIndexGuards;
                        k=indexLimitsComm[4];
                        j=indexLimitsComm[2];
                        i=indexLimitsComm[0];
