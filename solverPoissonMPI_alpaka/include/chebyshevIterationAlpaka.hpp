    ...
    {   
        ...
        */
        ...
        {   
            ...
            //memcpy(queue, bufZ, bufY, this->alpakaHelper_.extent_);
            //memcpy(queue, bufY, bufW, this->alpakaHelper_.extent_);
        }
...
template <int DIM, typename T_data, int tolerance, int maxIteration, bool isMainLoop, bool communicationON, typename T_Preconditioner>
class ChebyshevIterationAlpaka2 : public IterativeSolverBase<DIM,T_data,maxIteration>{
    public:

    ChebyshevIterationAlpaka2(const BlockGrid<DIM,T_data>& blockGrid, const ExactSolutionAndBCs<DIM,T_data>& exactSolutionAndBCs, 
                        CommunicatorMPI<DIM,T_data>& communicatorMPI, const AlpakaHelper<DIM,T_data>& alpakaHelper):
        IterativeSolverBase<DIM,T_data,maxIteration>(blockGrid,exactSolutionAndBCs,communicatorMPI),
        alpakaHelper_(alpakaHelper),
        theta_( (this->eigenValuesGlobal_[0]*rescaleEigMin + this->eigenValuesGlobal_[1]*rescaleEigMax) * 0.5 * (1.0 + epsilon) ),
    ...
    void operator()(T_data fieldX[], T_data fieldB[],MatrixFreeOperatorA<DIM,T_data>& operatorA)
    {
        ...
        auto bufXMdSpan = alpaka::experimental::getMdSpan(fieldXDev);
        auto fieldBDev = alpaka::allocBuf<T_data, Idx>(this->alpakaHelper_.devAcc_, this->alpakaHelper_.extent_);
        auto bufBMdSpan = alpaka::experimental::getMdSpan(fieldBDev);
        ...
        memcpy(queue, fieldBDev, fieldBHostView, this->alpakaHelper_.extent_);
        ...
        */
        ...
        memcpy(queue, fieldBHostView, fieldBDev, this->alpakaHelper_.extent_);
        auto endSolver = std::chrono::high_resolution_clock::now();
